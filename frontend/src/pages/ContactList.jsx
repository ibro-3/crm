...
import { contacts as api } from '../services/api';
...
export default function ContactList() {
  ...
  const loadContacts = async (searchTerm = '') => {
    ...
    setError(null);
    try {
      const data = await api.list(searchTerm ? { search: searchTerm } : {});
      setContacts(data);
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <Link to="/contacts/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-2 w-fit">
          <Plus size={20} />
          Add Contact
        </Link>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-12 text-center">
                    ...
                    <Link to="/contacts/new" className="text-blue-600 hover:underline mt-2 inline-block">
                  ...
                  <tr key={contact.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link to={`/contacts/${contact.id}`} className="text-blue-600 hover:bg-blue-50 p-2 rounded-lg" title="Edit">
                          <Edit2 size={16} />
                        </Link>
                        <button onClick={() => handleDelete(contact.id)} className="text-red-600 hover:bg-red-50 p-2 rounded-lg" title="Delete">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
